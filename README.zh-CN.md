...
1. 从 GitHub Release 下载 `arxiv-interest-daily-v1.2.5.xpi`。
2. 打开 Zotero。
3. 进入 `工具 -> 插件`。
4. 点击齿轮按钮。
5. 选择 `从文件安装附加组件...`。
...
`npm run build` 会根据当前 manifest 版本和 XPI SHA-256 重新生成 `updates.json`。发布新版本时，将生成的 XPI 上传到对应 GitHub Release，例如 tag `v1.2.5`。
